  ...
  return (
    <>
      ...
      <Box maxW="1090px" m="auto">
        ...
        <Box p="8" bgColor="gray.50" boxShadow="md">
          ...
          <Box ref={orderRef} p="8">
            ...
            <Flex justify="space-between" w="100%">
              ...
              <Flex flexDir="column" mx="4">
                ...
                <Text fontSize="x-small">
                  {`${profile.data.data.telefone} ${profile.data.data.celular && ` / ${profile.data.data.celular}`}`}
                </Text>
                <Box>
                  <Text fontSize="x-small">{profile.data.address.endereco} - {profile.data.address.bairro}</Text>
                  <Text fontSize="x-small">{profile.data.address.cidade}/{profile.data.address.estado} - {profile.data.address.cep}</Text>
                </Box>
              </Flex>
              <Flex flexDir="column" justify="center">
                <Flex align="center">
                  <Text mr="2">Número do pedido:</Text>
                  <Text fontSize="large" fontWeight="bold">{order.data.data.numero_pedido}</Text>            
                </Flex>
                <Flex align="center">
                  <Text mr="2">Data de entrega:</Text>
                  <Text fontSize="large" fontWeight="bold">{handleFormatDate(order.data.data.data_entrega)}</Text>            
                </Flex>
              </Flex>
            </Flex>
            ...
            <Flex flexDir="column">
              <Flex justify="space-between">
                ...
                { user.data.data.cpf_cnpj &&
                    <Box>
                      <Text fontSize="x-small" fontWeight="semibold">{user.data.data.razao_social ? 'CNPJ' : 'CPF'}</Text>
                      <Text fontSize="sm">{user.data.data.cpf_cnpj}</Text>
                    </Box>
                }
              </Flex>
              <Flex justify="space-between">
            ...
            <Flex justify="space-between">
              <Box>
                ...
                <Text fontSize="sm">{address.data.data.endereco}</Text>
              </Box>
              <Box>
                <Text fontSize="x-small" fontWeight="bold">Bairro:</Text>
                <Text fontSize="sm">{address.data.data.bairro}</Text>
              </Box>
              <Box>
                <Text fontSize="x-small" fontWeight="bold">CEP:</Text>
                <Text fontSize="sm">{address.data.data.cep}</Text>
              </Box>
              <Box>
                <Text fontSize="x-small" fontWeight="bold">Cidade/UF:</Text>
                <Text fontSize="sm">{address.data.data.cidade}/{address.data.data.estado}</Text>
              </Box>
            </Flex>

            <Flex mt="4" align="center">
              <Text fontSize="sm" fontWeight="bold" mr="2">Condição de pagamento:</Text>
              <Text>{order.data.data.condicao_pagamento}</Text>
            </Flex>
